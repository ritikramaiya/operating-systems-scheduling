  ...
  for (;;)
  {
    ...
    if (round_robin==1)
    {
      for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
      {
        ...
        {
          if ((proc1->state == RUNNABLE) && (highPriority->priority < proc1->priority))
            highPriority = proc1;
        }
...
void set_priority(int pid_para, int priority_para)
{
  struct proc *proc;
        acquire(&ptable.lock);
        for(proc = ptable.proc; proc < &ptable.proc[NPROC]; proc++){
          if(proc->pid == pid_para){
                        proc->priority = priority_para;
                        break;
                }
        }
        release(&ptable.lock);
